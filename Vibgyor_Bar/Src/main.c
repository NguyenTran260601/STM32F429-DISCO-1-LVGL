...
void LTDC_Pin_Init(void)
{
	...
	for(int i = 0 ; i < total_ltdc_pins ;i++){
		REG_SET_VAL(ltdc_io_ports[i]->MODER,2U,0x3U,(ltdc_pins[i] * 2U));		/*mode*/
		REG_CLR_BIT(ltdc_io_ports[i]->OTYPER,ltdc_pins[i]);						/*Output type*/
		REG_SET_VAL(ltdc_io_ports[i]->OSPEEDR,2U,0x3U,(ltdc_pins[i] * 2U));		/*speed*/
		if(ltdc_pins[i] < 8)
...
void LTDC_Init(void)
{
	...
	REG_SET_VAL(pLTDC->AWCR,(BSP_LCD_HSW + BSP_LCD_HBP + BSP_LCD_WIDTH-1),0xFFFU,LTDC_AWCR_AAW_Pos);
	uint32_t total_width = BSP_LCD_HSW + BSP_LCD_HBP + BSP_LCD_WIDTH + BSP_LCD_HFP - 1;
	...
	//Configure the background color(GREEN)
	REG_SET_VAL(pLTDC->BCCR,0x00FF00U,0xFFFFFFU,LTDC_BCCR_BCBLUE_Pos);
...
void System_Clock_Config(void)
{
	...
	REG_SET_VAL(pFlash->ACR,0x5U,0xFU,FLASH_ACR_LATENCY_Pos); //5 SW
	...
	//2. Setting up LCD_CLK using PLLSAI
	...
	//3. Setting up AHB and APBx clocks
