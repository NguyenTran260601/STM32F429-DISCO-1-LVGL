...
uint16_t Convert_RGB888_to_RGB666(uint32_t rgb888)
{
    uint16_t r,g,b;
	r = (rgb888 >> 18) & 0x3FU;
	g = (rgb888 >> 10) & 0x3FU;
	b = (rgb888 >> 2)  & 0x3FU;
	return (uint16_t)((r << 12) | (g << 6) | b);
}
...
void write_to_fb_rgb888(uint32_t *fb_ptr, uint32_t n_pixels, uint32_t rgb888)
{
	while(n_pixels--){
		*fb_ptr = rgb888;
		 fb_ptr++;
...
void write_to_fb_rgb666(uint32_t *fb_ptr, uint32_t n_pixels, uint32_t rgb666)
{
	while(n_pixels--){
		*fb_ptr = rgb666;
		 fb_ptr++;
